...
|------|-----------|--------|------------|
| **Agent hallucination** | Medium | High | Hypothesis loop with 5-point verification; confidence scoring; human review for high-impact | [General Research](./General%20-%20deep-research-report.md) |
| **MCP server outage** | Medium | High | Hard gate check (Step 0.3); graceful degradation; retry logic | Current implementation |
| **Prompt injection via Jira** | Low | Critical | Treat ticket data as untrusted; structured extraction only | [OWASP LLM Top 10](https://owasp.org/www-project-top-10-for-large-language-model-applications/) |
| **Token cost explosion** | Medium | Medium | Model tiering; max 5 hypothesis iterations; token budgets per mode | [Anthropic Multi-Agent Costs](./Plugins%20-%20deep-research-report.md) |
| **Cross-platform drift** | High | Medium | Shared core engine; integration tests on core changes | Monorepo design |
| **Secrets leak** | Low | Critical | Never in code; install-time prompts; CI secret scanning | Current `install.sh` |
| **Knowledge poisoning** | Low | High | Human review before memory updates; versioned MEMORY.md | [Research](./General%20-%20deep-research-report.md) |
| **Platform API changes** | Medium | Medium | Pin platform SDK versions; adapter isolation | Independent adapters |
