...
- `PUBLISH_TO` — Where to publish: "jira", "slack", or "both"
- `SLACK_CHANNEL` — Channel name or thread URL (if Slack chosen)
- `REPLY_TO_COMMENT_ID` — (Optional) Jira comment ID to reply to with quoting
...
### Step 2: Preview and confirm with user

First, show a PREVIEW of the formatted content — both the Jira wiki markup version and the Slack mrkdwn version. Display them in clearly labeled sections so the user can review both.

Then use `AskUserQuestion` to ask:

**Question 1:** "Where should I publish this?"
Options:
- "Jira comment on [TICKET-ID]"
- "Slack thread (I'll specify channel/thread)"
- "Both Jira and Slack"

**Question 2:** "Are you happy with the content?"
Options:
- "Yes, publish as-is"
- "No, I want to edit it first"

**If user selects "edit"**, enter an edit loop:
1. Ask what to change (free text via AskUserQuestion)
2. Apply the edit to the formatted content (both Jira and Slack versions)
3. Show updated preview
4. Re-ask confirmation: "Publish now?" / "Edit more"

Only proceed to Step 3 after explicit "publish" confirmation.

**Slack thread reply support:** If the user provides a Slack thread URL (e.g., `https://wix.slack.com/archives/C123/p1234567890`), parse:
- `channel_id` from the `/archives/<channel_id>/` segment
- `thread_ts` from the `p<timestamp>` segment (insert a `.` before the last 6 digits)
Then use `slack_reply_to_thread` instead of `slack_post_message`.
...
**Reply-to-comment mode:** If `REPLY_TO_COMMENT_ID` is provided in the inputs:
1. Fetch the original comment from the Jira ticket (search `REPORT_CONTENT` or `BUG_CONTEXT_REPORT` for the comment text)
2. Format the reply with a quote of the original:
```
bq. [~original-author] wrote: [first 2 lines of original comment]

[Our reply here]
```
3. Post as a new comment (Jira API doesn't support threaded replies, so quoting is the best we can do)
