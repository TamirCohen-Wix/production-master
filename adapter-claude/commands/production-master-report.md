# Production Master Report — Submit Feedback via GitHub Issue
...
```bash
# Plugin version
cat /Users/tamirc/Projects/production-master/VERSION 2>/dev/null || echo "unknown"
...
ls ~/.claude/production-master/domains/${REPO_NAME}/domain.json 2>/dev/null
```
...
1. **Pipeline run summary** — which agents ran, which completed, which failed
...
4. **Agent errors** — agents that produced empty or malformed output
...
- **Agents that ran:** {list of agents from trace files}
- **Agents that failed/errored:** {list, or "none"}
...
#### Agent Errors
{Any agent errors — agent name, error type, brief excerpt}

#### Quality Gate Issues
{Any verifier/skeptic rejections — reason, confidence score}

### Environment
- **Plugin version:** {PLUGIN_VERSION}
- **OS:** {OS_INFO}
- **Domain config:** {DOMAIN_CONFIG path or "none"}
- **MCP servers:** {list which servers were checked in Step 0.3, pass/fail status}
...
If no `DEBUG_DIR` was found, use this simpler template:
...
*No debug directory found — run `/production-master` first to generate trace data.*
```
...
If `USER_DESCRIPTION` is short (fewer than 10 words), ask the user:
...
If a recent debug directory exists, ask the user:
> "I found a recent investigation run. Would you like to reference it in the enhancement request for context?"

If yes, read `findings-summary.md` and `report.md` to understand the context.
...
### Current Behavior
{What currently happens — from user input or inferred from debug context}
